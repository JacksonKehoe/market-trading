...
| `app/dashboard` | A read-only Flask app (`app.py`, one route) showing a live comparison across every configured strategy: a summary table, a combined equity-curve chart, and combined holdings/signals/risk/transactions tables tagged by strategy — all read from `SqlTradeRepository` plus one live price/signal check per strategy per symbol. Every strategy tag shown in the UI is a cosmetic display label (`_STRATEGY_DISPLAY_LABELS`: `sma`→SMA, `rsi`→RSI, `macd`→MACD, `sma_sentiment`→SENTIMENT) rather than the technical `Strategy.name` (e.g. `sma_crossover_20_50`) — the technical name is still what's used for every database lookup, just never rendered. The Strategy Signals table defaults to hiding HOLD rows client-side (an unchecked "Show all signals" toggle reveals them) since a signal that's been HOLD for days isn't actionable. Also adds a `BENCHMARK_SYMBOL` comparison row and chart series (same shared `compute_benchmark_curve`, same 7-day minimum lookback as the email reports), styled distinctly and excluded from the signal-filter dropdown since it isn't a strategy. Runs on `127.0.0.1` only; never writes to the database or places a trade. | `models`, `config`, `data`, `database`, `risk`, `strategies`, `reporting` |
...
- **The equity-curve chart's range selector/slider is a chart-only fix,
  not a table-level one.** The 7-day (or longer) minimum lookback above
  means a brand-new strategy's benchmark line legitimately shows the
  benchmark's return over a window the strategy itself wasn't trading in
  yet — e.g. SPY at "+13%" while a same-day strategy is still flat at its
  starting equity. The range selector lets the chart be narrowed to the
  period the strategy has actually been live, so the *visual* comparison
  is fair. The numeric `return_pct` in the Strategy Comparison table
  above the chart is **not** recomputed when the chart is zoomed — it's
  still the return over the full padded window — so treat the table as
  "benchmark's return over its lookback window," and the zoomed chart as
  "how the benchmark and strategies actually moved together," not the
  same number restated two ways.
